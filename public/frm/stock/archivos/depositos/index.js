mostrar_nombre_programa("Depositos");
...
function editar_linea(xthis) {
   // modificado = false
    id_deposito = xthis.parentElement.parentElement.getAttribute('data-id_deposito')
...
function eliminar_linea(xthis) {
    id_deposito_eliminar = xthis.parentElement.parentElement.getAttribute('datos-id_deposito')
...
async function buscar_depositos() {
    let buscar = document.getElementById('buscar').value;
    let url = `/api/v1/depositos?buscar=${buscar}`;
    var parametros = {
        method: "GET",
        headers: {
    ...
    const tbody = document.getElementById('tbody-datos-depositos');
    tbody.innerText = '';
    let lineas = '';
    if (json.status === 200) {
        for (let item in json.datos) {
            let linea = `<tr data-id_deposito=${json.datos[item].id} data-id_sucursal=${json.datos[item].id_sucursal}>
...
async function guardar_modificar() {
    let url = `/api/v1/depositos/${id_deposito}`;
    let nombre = document.getElementById('nombre').value;
    //let sucursal = document.getElementById('sucursal').value;
    let data = {
        nombre: nombre,
...
async function guardar_eliminar() {
    let url = `/api/v1/depositos/${id_deposito_eliminar}`;
    let data = {};
    let parametros = {
        method: "DELETE",
        headers: {
