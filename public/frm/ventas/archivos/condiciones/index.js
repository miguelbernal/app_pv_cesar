mostrar_nombre_programa("Condiciones");
...
function editar_linea(xthis){
    id_condicion = xthis.parentElement.parentElement.getAttribute('data-id_condicion')
...
function eliminar_linea(xthis){
    id_condicion_eliminar = xthis.parentElement.parentElement.getAttribute('data-id_condicion')
...
//Llamar al servidor
async function buscar_condiciones() {
    let buscar = document.getElementById('buscar').value;
    let url = `/api/v1/condiciones?buscar=${buscar}`;
    var paramentros = {
        method:"GET",
        headers: {
    ...
    console.log(json)
    const tbody = document.getElementById('tbody-datos-condiciones');
    tbody.innerText = '';
    let lineas = '';
    if(json.status === 200) {
        for(let item in json.datos) {
            let linea = `<tr data-id_condicion=${json.datos[item].id}>
...
async function guardar_modificar(){
    let url = `api/v1/condiciones/${id_condicion}`;
    let nombre = document. getElementById('nombre').value;
...
async function guardar_eliminar(){
    let url = `api/v1/condiciones/${id_condicion_eliminar}`;
