...
function editar_linea(xthis){
    id_forma_pago = xthis.parentElement.parentElement.getAttribute('data-id_forma_pago')
...
function eliminar_linea(xthis){
    id_forma_pago_eliminar = xthis.parentElement.parentElement.getAttribute('data-id_forma_pago')
...
//Llamar al servidor
async function buscar_formas_pagos() {
    let buscar = document.getElementById('buscar').value;
    let url = `/api/v1/formas_pagos?buscar=${buscar}`;
    var paramentros = {
        method:"GET",
        headers: {
    ...
    console.log(json)
    const tbody = document.getElementById('tbody-datos-formas_pagos');
    tbody.innerText = '';
    let lineas = '';
    if(json.status === 200) {
        for(let item in json.datos) {
            let linea = `<tr data-id_forma_pago=${json.datos[item].id}>
...
async function guardar_modificar(){
    let url = `api/v1/formas_pagos/${id_forma_pago}`;
    let nombre = document. getElementById('nombre').value;
...
async function guardar_eliminar(){
    let url = `api/v1/formas_pagos/${id_forma_pago_eliminar}`;
